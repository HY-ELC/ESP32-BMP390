...
void FFT_AddSample(float sample)
{
    ...
    if (SampleIndex >= FFT_SAMPLES)
    {
        ...
        /* 风险等级 */
        if (EquivalentDB < 120.0f)          RiskLevel = LEVEL_SAFE;
        else if (EquivalentDB < 130.0f)     RiskLevel = LEVEL_NOTICE;
        else if (EquivalentDB < 140.0f)     RiskLevel = LEVEL_WARNING;
